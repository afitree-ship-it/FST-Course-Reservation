    ...
    try {
      ...
      if (result.success) {
        ...
        if (!result.isFastResolved) {
           ...
           // If fast resolved, let the background fetchPromise trigger line notification when it finishes!
...
pattern = re.compile(r'export async function submitReservation\(data: Partial<ReservationRequest>\): Promise<\{ success: boolean; data\?: ReservationRequest; error\?: string \}> \{.*?  \}', re.DOTALL)
match = pattern.search(content)
if match:
    new_content = content[:match.start()] + replacement + content[match.end():]
    with open('src/services/api.ts', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Successfully patched submitReservation.")
else:
